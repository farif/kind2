...
module Hashtbl = Hashtbl.Make (BtorIdent)

(* Set over identifiers *)
module Set = Set.Make (BtorIdent)

(* Map over identifiers *)
module Map = Map.Make (BtorIdent)
...
let rec pp_print_index safe ppf = function 
  ...
  | h :: tl -> 
    ...
    (* Pretty-print valid Btor2 identifiers? *)
    if safe then 
